...
const Subscription2 = () => {
        ...
        <div className="bg-[var(--card)] rounded-[28px] p-6 md:p-8 space-y-10  ">
          <section>
            ...
            <div className="flex overflow-x-auto pb-4 gap-4 hide-scrollbar snap-x">
              {/* Package Items */}
              {[
                { id: 'Mini', meons: 99, price: '00.99', oldPrice: null, discount: null, tag: null, image: mini },
                { id: 'Small', meons: 500, price: '4.49', oldPrice: null, discount: '-10%', tag: null, image: small },
                { id: 'Medium', meons: 1000, price: '7.99', oldPrice: null, discount: '-20%', tag: null, image: medium },
                { id: 'XL', meons: 2000, price: '12.99', oldPrice: null, discount: '-35%', tag: 'BEST VALUE', image: XL },
                { id: 'Large', meons: 5000, price: '24.99', oldPrice: null, discount: '-50%', tag: 'Most Popular', highlight: true, image: large },
              ].map((pkg) => (
                <div
                  key={pkg.id}
                ...
                >
                  ...
                  <div className="text-center">
                    <h4 className="font-bold text-sm">{pkg.id}</h4>
                    <p className="text-xs text-gray-500 mb-2">{pkg.meons} meons</p>
          ...
          <section>
            <h3 className="font-bold text-lg mb-4 text-[var(--text-dim)]">Visibility Boosts</h3>
            <div className=" rounded-2xl p-6  flex flex-col md:flex-row gap-8">
              ...
              <div className="flex-1 bg-[var(--bg-card)]/10 border border-[var(--border)]  p-5 rounded-2xl">
                <h4 className="font-bold mb-4 text-[var(--text-dim)] ">Single Boosts</h4>
                <div className="space-y-3">
                  {[
                    { id: '30 min', cost: '199', tag: null },
                    { id: '1 hour', cost: '349', tags: [{ text: 'Recommend', color: 'bg-fuchsia-500' }, { text: 'Save 12%', color: 'bg-green-500' }] },
                    { id: '2 hours', cost: '599', tags: [{ text: 'Popular', color: 'bg-blue-600' }, { text: 'Save 20%', color: 'bg-green-500' }] },
                    { id: '4 hours', cost: '999', tags: [{ text: 'Save 30%', color: 'bg-green-500' }] },
                  ].map((item) => (
                    <div key={item.id} className="relative">
                      {item.tags && (
                        <div className="absolute -top-3 left-4 flex gap-1 z-10 text-[var(--text-dim2)]">
                          {item.tags.map((tag, idx) => (
                            <span key={idx} className={`text-[9px] text-[var(--text-dim2)] px-1.5 py-0.5 rounded-sm ${tag.color}`}>{tag.text}</span>
                          ))}
                        </div>
                      )}
                      <div
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3">
                          <span className="text-sm text-[var(--text-dim)]">{item.cost} meons</span>
              ...
              <div className="flex-1 relative bg-[var(--bg-card)]/10 border border-[var(--border)]  p-5 rounded-2xl">
                ...
                <h4 className="font-bold mb-4 mt-2 text-[var(--text-dim)]">Boost Packs</h4>
                <div className="space-y-3 ">
                  {[
                    { id: 'Pack Charm', desc: '3 x 30min', cost: '499', tag: 'Save 15%' },
                    { id: 'Pack Seduction', desc: '5 x 30min', cost: '799', tag: 'Save 20%' },
                    { id: 'Pack Irresistible', desc: '10 x 30min', cost: '1499', tag: 'Save 35%' },
                  ].map((item) => (
                    <div key={item.id} className="relative mt-5 text-[var(--text-dim)]">
                      {item.tag && (
                        <div className="absolute -top-3 left-4 z-10">
                          <span className={`text-[9px] text-[var(--text-dim2)] px-1.5 py-0.5 rounded-sm bg-green-500`}>{item.tag}</span>
                        </div>
                      )}
                      <div
                      ...
                      >
                        <div>
                          ...
                          <div className="text-[11px] text-[var(--text-dim)] ">{item.desc}</div>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="text-sm text-[var(--text-dim)] ">{item.cost} meons</span>
              ...
              {/* VIP Mode */}
              <div className="flex-1 bg-[var(--bg-card)]/10 border border-[var(--border)]  p-5 rounded-2xl">
                <h4 className="font-bold mb-4 mt-2 text-[var(--text-dim)]">VIP Mode / Incognito</h4>
                <div className="space-y-3">
                  {[
                    { id: '24 hours', cost: '199' },
                    { id: '7 days', cost: '599' },
                    { id: '30 days', cost: '999' },
                  ].map((item) => (
                    <div
          ...
          <section>
            <div className="bg-[var(--bg-card)]/10 rounded-2xl p-6 flex flex-col md:flex-row gap-8">
              ...
              <div className="flex-1">
                <h4 className="font-bold mb-4">Roses & Bouquets</h4>
                <div className="space-y-3">


                  {[
                    { id: 'Single Rose', desc: '5 roses', cost: '20', tag: null },
                    { id: 'Small Bouquet', desc: '5 roses', cost: '99', tag: null },
                    { id: 'Medium Bouquet', desc: '12 roses', cost: '200', tag: 'Save 17%' },
                    { id: 'Large Bouquet', desc: '12 roses', cost: '375', tag: 'Save 25%' },
                  ].map((item) => (
                    <div key={item.id} className="relative mt-4">
                      {item.tag && (
                        <div className="absolute -top-3 left-4 z-10">
                          <span className={`text-[9px] text-[var(--text)] px-1.5 py-0.5 rounded-sm bg-green-500`}>{item.tag}</span>
                        </div>
                      )}
                      <div
                      ...
                      >
                        <div>
                          <div className="text-sm text-[var(--text)] ">{item.id}</div>
              ...
              {/* Question Paths */}
              <div className="flex-1">
                <h4 className="font-bold mb-1 text-[var(--text-dim)]">Question Paths</h4>
                <p className="text-xs text-[var(--text-dim)]  mb-4">Includes: questions + chat + compatibility visibility</p>
                <div className="space-y-3 mt-5">
                  {[
                    { id: 'Essential Path', desc: '30 Questions', cost: '9.99', tag: null },
                    { id: 'Serenity Path', desc: '40 Questions', cost: '19.99', tag: 'Save 17%' },
                    { id: 'Elite Path', desc: '40 Questions', cost: '29.99', tag: 'Save 25%' },
                  ].map((item) => (
            ...
            <div className=" rounded-2xl p-6  flex flex-col md:flex-row gap-8 border border-[var(--border)] bg-[var(--bg-card)]/10">
              ...
              <div className="flex-1 bg-[var(--bg-card)]/10 border border-[var(--border)]  p-4 rounded-xl">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h4 className="font-bold leading-tight text-[var(--text-dim)]">Lucas Time</h4>
                    <p className="text-xs text-[var(--text-dim2)] ">AI coach</p>
                  </div>
                </div>
  ...
  {[
    { id: '1 Hour', cost: '4.99' },
    { id: '2 Hours', cost: '7.99' },
    { id: '5 Hours', cost: '12.99' },
    { id: '10 Hours', cost: '19.99' },
  ].map((time) => (
    <div
      key={time.id}
      onClick={() =>
    ...
    >
      ...
      />
              ...
              <div className="flex-1 flex flex-col justify-center space-y-3 bg-[var(--bg)]">
                {[
                  {
                    title: 'Lucascope Guide',
                    desc: '+1h/month of astrology and compatibility',
                    price: '7.99',
                    image: lucascope,
                  },

                  {
                    title: 'Lucascope Infinity',
                    desc: 'Save 2 months on yearly plan',
                    price: '19.99',
