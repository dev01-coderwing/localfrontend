    ...
    <div className={`flex items-center justify-between p-4  bg-[var(--bg-background)]  border border-transparent hover:border-orange-100 rounded-2xl transition-all duration-200 ${!isLast ? 'mb-2' : ''}`}>
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col">
          <span className="font-bold  text-[var(--text)] text-sm leading-tight">{t(`profileNotification.${item.id}_title`)}</span>
          <span className="text-xs text-[var(--text-dim2)] font-medium mt-0.5">{t(`profileNotification.${item.id}_desc`)}</span>
        </div>
      </div>
      <Toggle
        enabled={isEnabled}
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-8">
        <section className="bg-[var(--bg-background)] rounded-[24px] p-6 md:p-10 shadow-inner overflow-hidden">
          ...
          <header className="flex items-center gap-4 mb-8">
            ...
            <h1 className="text-2xl font-semibold">{t('profileNotification.title')}</h1>
          </header>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              <article className="bg-[var(--bg-card)]/10 p-6 md:p-8 rounded-[32px] border border-[var(--border)] shadow-sm">
                <div className="mb-8 ">
                  <h2 className="text-[18px]  font-medium mb-2">{t('profileNotification.header_title')}</h2>
                  <p className="text-sm  text-black-500 font-semibold text-[16px]">{t('profileNotification.sub_title')}</p>
                </div>
                ...
                <footer className="mt-16 text-center justify-center ">
                  <p className="text-[12px] text-black-400 mx-auto opacity-80">
                    {t('profileNotification.footer_note')}
                  </p>
                </footer>
              </article>
            </div>
